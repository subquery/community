...
AcalaとKaruraは、DeFiの将来のための巨大なビジョンを表しているので、エコシステムのための貴重なツールを構築する大きな機会があります。 この課題には、開発者は、 [Zapper.fi](http://zapper.fi/)に触発されたKarura/Acalaコミュニティ向けのDeFiツールを作成する必要がありました。 SubQueryのデータインデックスサービスを利用する と [defisaver.com](http://defisaver.com/)。
